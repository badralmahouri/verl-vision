...
class AgentData:
    ...
    def __init__(
        ...
        self.tool_execution_errors: int = 0  # Track number of tool execution failures
        self.user_turns = 0
...
class ToolAgentLoop(AgentLoopBase):
    ...
    async def run(self, sampling_params: dict[str, Any], **kwargs) -> AgentLoopOutput:
        ...
        # Try to get tools_kwargs from top-level, then from metadata
        tools_kwargs = kwargs.get("tools_kwargs", {})
        if not tools_kwargs or tools_kwargs == {}:
            # Try getting from metadata
            metadata = kwargs.get("metadata", {})
            if isinstance(metadata, dict) and "tools_kwargs" in metadata:
                tools_kwargs = metadata.get("tools_kwargs", {})
        
        # Convert pyarrow/numpy dicts to regular Python dicts if needed
        if tools_kwargs and hasattr(tools_kwargs, 'as_py'):
            tools_kwargs = tools_kwargs.as_py()
        if tools_kwargs and hasattr(tools_kwargs, 'tolist'):
            tools_kwargs = dict(tools_kwargs)


        logger.error(f"DEBUG tools_kwargs (after conversion): {tools_kwargs}")
        logger.error(f"DEBUG tools_kwargs type: {type(tools_kwargs)}")
        ...
        output.extra_fields.update({
            ...
            "tool_execution_errors": agent_data.tool_execution_errors,
        })
        return output
    ...
    async def _handle_generating_state(
        ...
        if agent_data.tool_calls:
            for tc in agent_data.tool_calls:
                logger.error(f"TOOL USED: {tc.name}")
    ...
    async def _handle_processing_tools_state(self, agent_data: AgentData) -> AgentState:
        ...
        for tool_response, tool_reward, tool_result in responses:
            ...
            if tool_result.get("tool_execution_error", False):
                agent_data.tool_execution_errors += 1
    ...
    async def _call_tool(
        ...
        Returns:
            ...
            - float: The tool reward (0.0 on error)
            - dict: Result dict, includes {"tool_execution_error": True} on failure
        """
        tool, instance_id = None, None
        try:
            # TODO: append malformed tool_call to the prompt: invalid function name or arguments
            tool_name = tool_call.name
            logger.error(f"TOOL USED: {tool_name}")
            tool_args = json.loads(tool_call.arguments)
            tool = self.tools[tool_name]
            kwargs = tools_kwargs.get(tool_name, {})
            create_kwargs = kwargs.get("create_kwargs", {})
            # Unpack create_kwargs so image=... is passed as keyword arg
            create_result = await tool.create(**create_kwargs)
            # Handle both return types: str or tuple[str, ToolResponse]
            if isinstance(create_result, tuple):
                instance_id, _ = create_result
            else:
        ...
        # Handle both dict and object responses from tool.execute()
        if isinstance(tool_execution_response, dict):
            tool_response_text = tool_execution_response.get("text", "")
        else:
        ...
        # Add multimedia data if present (handle both dict and object)
        for attr_name in ["image", "video"]:
            if isinstance(tool_execution_response, dict):
                attr_value = tool_execution_response.get(attr_name)
            else:
                attr_value = getattr(tool_execution_response, attr_name, None)
