...
# SamplingMetadata import fallback - THREE levels of fallback
try:
    # First try: vLLM < 0.11 location
    from vllm.model_executor.sampling_metadata import SamplingMetadata
except ImportError:
    try:
        # Second try: vLLM >= 0.11 location
        from vllm.sampling_metadata import SamplingMetadata
    except ImportError:
        # Third try: vLLM dev versions that removed it entirely
        # Create a dummy class that satisfies the type hints
        class SamplingMetadata:
            """Dummy SamplingMetadata for vLLM versions that removed this class."""
            pass
        print("WARNING: Could not import SamplingMetadata from vLLM, using dummy class")
        # Make it available in the vllm namespace for any other code that might import it
        import vllm
        vllm.sampling_metadata = type(sys)('sampling_metadata')
        vllm.sampling_metadata.SamplingMetadata = SamplingMetadata
...
class vLLMRollout(BaseRollout):
    def __init__(self, model_path: str, config: DictConfig, tokenizer, model_hf_config, **kwargs):
        ...
        # Build engine kwargs dynamically based on vLLM version
        engine_kwargs = {}
        ...
        # ==================== vLLM 0.11+ Compatibility ====================
        # Build LLM kwargs with version-specific parameters
        llm_kwargs = {
            "model": model_path,
        ...
        # vLLM 0.11+ removed some parameters and changed defaults
        if VLLM_VERSION < (0, 11):
            # Legacy parameters for vLLM < 0.11
            llm_kwargs.update({
            ...
            # vLLM 0.11+ changes
            # These parameters might be deprecated or moved to engine_kwargs
            if hasattr(config, 'disable_log_stats'):
                llm_kwargs["disable_log_stats"] = config.disable_log_stats
            if hasattr(config, 'free_cache_engine') and config.free_cache_engine:
                logger.warning("vLLM 0.11+ may not support enable_sleep_mode parameter directly")
        # =================================================================

        self.inference_engine = LLM(**llm_kwargs)

        # Offload vllm model to reduce peak memory usage
        # vLLM 0.11+ might have different sleep API
        if config.free_cache_engine and hasattr(self.inference_engine, 'sleep'):
            try:
    ...
    def generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        with self.update_sampling_params(**sampling_kwargs):
            ...
            for output in outputs:
                for sample_id in range(len(output.outputs)):
                    ...
                    if self.config.calculate_log_probs:
                        curr_log_prob = []
                        logprobs = output.outputs[sample_id].logprobs
                        if logprobs:
                            for i, logprob_dict in enumerate(logprobs):
                                if logprob_dict and response_ids[i] in logprob_dict:
            ...
            if self.config.calculate_log_probs:
                ...
                ).to(idx.device, dtype=torch.float32)
...
def _monkey_patch_compute_logits(model, vocab_size: int):
    """Monkey patch compute_logits to mask out unused vocab tokens."""
    if not hasattr(model, 'compute_logits'):
        logger.warning("Model does not have compute_logits method, skipping monkey patch")
        return
    ...
    def compute_logits(
        ...
        if logits is not None and logits.size(-1) > vocab_size:
...
class vLLMAsyncRollout:
    ...
    def _loop_forever(self):
        while True:
            try:
                ...
                logger.error(f"Error in vLLM async loop: {e}", exc_info=True)
    ...
    def init_worker(self, all_kwargs: list[dict[str, Any]]):
        ...
        all_kwargs[0]["rank"] = int(os.environ.get("RANK", 0))
